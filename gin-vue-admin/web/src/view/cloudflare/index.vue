  ...
  <div>
    <div class="gva-table-box">
      <div class="gva-btn-list">
        <el-button type="primary" icon="plus" @click="buttentest">新增</el-button>
      </div>
      <el-table ref="multipleTable" :data="tableData" style="width: 100%" tooltip-effect="dark" row-key="ID">
        <el-table-column type="selection" width="55" />
        <el-table-column align="left" label="ID" prop="id" width="300" />
        <el-table-column align="left" label="域名" prop="name" width="200" />
        <el-table-column align="left" label="NameServer" prop="name_servers" width="370" />
        <el-table-column align="left" label="状态" prop="status" width="120">
          <template #default="scope">
            <el-tag :type="scope.row.status === 'active' ? 'success' : 'danger'" disable-transitions>{{ scope.row.status
            }}</el-tag>
          </template>
        </el-table-column>
...
import { getCfDomainList } from '@/api/cloudflareapi'
...
const buttentest = async () => {
  const data = await getCfDomainList()
}

// 查询
const getTableData = async () => {
  const table = await getCfDomainList()
  if (table.code === 0) {
